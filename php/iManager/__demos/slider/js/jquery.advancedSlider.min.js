    ...
    function AdvancedSlider(instance, options) {
        ...
        */
        function create() {
            ...
            if (self.settings.navigationArrows && slides.length > 1)
                createNavigationArrows();
                
            if (self.settings.navigationButtons && slides.length > 1)
                createNavigationButtons();
                
            if (self.settings.shadow)
                createShadow();
                
            if (self.settings.thumbnailsType == 'navigation' && slides.length > 1)
                createNavigationThumbnails();
                        
            if (self.settings.slideshowControls && slides.length > 1)
                createSlideshowControls();          
                
            if (slides.length < 2)
                self.settings.slideshow = false;
