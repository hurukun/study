...
exports.class.prototype = {
    ...
    handleRequest: function() {
        // var logEntry = this._req.method + ' ' + this._req.url;
        // if ( this._req.headers['user-agent']) {
        //     logEntry += ' ' + this._req.headers['user-agent'];
        // }
        // util.puts(logEntry);
        this._req.url = this._parseUrl(this._req.url);
        var handler = this.handlers[this._req.method];
        if (!handler) {
            this._res.writeHead(501);
            this._res.end();
        } else {
...
StaticServlet.prototype.handleRequest = function(req, res) {
    ...
    if (parts[parts.length - 1].charAt(0) === '.') return self.sendForbidden_(req, res, path);
    fs.stat(path, function(err, stat) {
        if (err) return self.sendMissing_(req, res, path);
...
StaticServlet.prototype.sendMissing_ = function(req, res, path) {
    ...
    res.write(
        '<p>The requested URL ' + escapeHtml(path) +
...
StaticServlet.prototype.sendFile_ = function(req, res, path) {
    ...
    res.writeHead(200, {
        'Content-Type': Config.mimes[path.split('.').pop()] || 'text/plain'
    });
    if (req.method === 'HEAD') {
        res.end();
    } else {
