package
{
	...
	public class Railings extends Sprite
	{
		...
		private function onInited( event:Event ):void{
			...
			this.url = 'w_mengchong.jpg';//param[ 'img' ];
			//this.xSample = param[ 'xSample' ]
			this.xSample = 128;
			this.ySample = 128;
			this.ampFactor = 4;
			this.prevMousePos = new Point( -1, -1 );
			this.imgData = [];
		...
		private function onImageLoaded( event:Event ):void{
			...
			circle.addEventListener( MouseEvent.MOUSE_DOWN, onEnterCircle );
			circle.bindEvents();
		...
		*/ 
		private function onMouseMove( event:MouseEvent ):void{
			...
			if( this.prevMousePos.x > 0 ){
				sample = this.traceMove( this.prevMousePos,curMousePos,4 );
		...
		private function onEnterCircle( evt: Event ):void{
			evt.target.removeEventListener( MouseEvent.MOUSE_DOWN, onEnterCircle);
			this.split( evt.target );
		}
		/*-------------------------------------------------------------------*/
...
//			}
		...
		private function split( circle:Object ):void{
			...
			for( var ii:int = 0; ii < children.length; ii++ ){
				...
				cRadius = children[ ii ].getRadius();
				children[ ii ].setColor( this.avgColor( cChild.x / this.ampFactor, cChild.y / this.ampFactor, cRadius/ this.ampFactor ) );
				children[ ii ].draw();
				this.addChild( children[ ii ] );
				this.cache.add( cChild.x,cChild.y,children[ ii ], false );
				if( cRadius > 4 ){
					children[ ii ].addEventListener( MouseEvent.MOUSE_DOWN, onEnterCircle );
					children[ ii ].bindEvents();
				}
			}
