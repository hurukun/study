package flash.shape
{
	...
	public class Circle
	{
		...
		public function checkIntersection (startPoint:Point, endPoint:Point ):Boolean {
			var edx:int = this.x - endPoint.x , 
				edy:int = this.y - endPoint.y, 
				sdx:int = this.x - startPoint.x, 
				sdy:int = this.y - startPoint.y, 
				r2:int = this.radius;
		...
		public function draw():Sprite{
			...
			this.sprite.graphics.beginFill( this.color, this.opacity );
			this.sprite.graphics.drawCircle( this.x, this.y, this.radius );
